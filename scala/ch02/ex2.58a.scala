...
private def isArithmeticExpression (exp: Expression, operator: String) = exp match {
  case head::tail => tail.head == operator
  case _ => false
}
...
object expressionParser extends JavaTokenParsers {
  ...
  def parentheticalExpression = "(" ~> operand ~ operator ~ operand <~ ")" ^^ { 
    case rand1 ~ op ~ rand2 => rand1 :: op :: rand2 :: Nil
  }
  def operand = exp
