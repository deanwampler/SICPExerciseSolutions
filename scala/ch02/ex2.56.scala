...
private def toExpr(tokens: Any*) = tokens.toList
...
private def extractFirstArg (exp: Expression) = exp.tail.head match {
  case l:List[_] => l
  case x => List(x)
}
// Assumes the expression contains at least two arguments/operands
private def extractSecondArg(exp: Expression) = exp.tail.tail.head match {
  case l:List[_] => l
  case x => List(x)
}
